...
/** Per-node spacing check — called by both the single-pass walker and collectViolations. */
export function visitSpacingNode(
  node: FigmaNode,
  nodePath: string,
  ...
  if (radii && radii.some((r) => r > 0)) {
    const bvRadii = node.boundVariables?.["rectangleCornerRadii"] as Record<string, unknown> | undefined;
  ...
  if (!node.layoutMode || node.layoutMode === "NONE") {
    if (node.x !== undefined && node.x !== 0) {
      ...
      if (!node.boundVariables?.["x"]) {
        violations.push({ nodePath, property: "x", rawValue: String(node.x), expected: "A semantic spacing variable" });
      }
    }
    if (node.y !== undefined && node.y !== 0) {
      totalSpacingProps++;
      if (!node.boundVariables?.["y"]) {
        violations.push({ nodePath, property: "y", rawValue: String(node.y), expected: "A semantic spacing variable" });
      }
    }
  }
...
function collectViolations(
  ...
  if (node.type !== "COMPONENT_SET") {
    totalSpacingProps += visitSpacingNode(node, nodePath, violations);
  }
