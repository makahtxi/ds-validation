...
export default function AuditDetailPage() {
  ...
  const componentsForMatrix = audit.components.map((c) => ({
    ...
    score: c.score,
    passedChecks: Object.values(c.result as Record<string, { status: string }>).filter(
      (r) => r.status === "pass",
    ).length,
    totalChecks: Object.keys(c.result as Record<string, unknown>).length,
    pageName: c.page_name,
    jsonPath: `components/${sanitizeComponentName(c.component_name)}.json`,
  }));

  const auditResultForMatrix = {
    meta: {
      figmaFileKey: audit.fileKey,
      figmaFileName: audit.fileName,
      auditedAt: audit.createdAt,
      pagesAudited: audit.selectedPages ?? [],
      conformanceChecks: CHECK_DEFS.map((d) => ({
        id: d.id,
        name: d.name,
        weight: d.weight,
      })),
    ...
    summary: {
      template: "audit_overview",
      params: { componentCount: componentsForMatrix.length, totalScore: audit.totalScore ?? 0 },
    },
    components: componentsForMatrix,
  };
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {audit.selectedPages?.length ?? 0} page{audit.selectedPages?.length !== 1 ? "s" : ""} audited
